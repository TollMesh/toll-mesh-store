...
- A raw WAL entry append (checksummed, length-prefixed, written to disk) takes ~1.6μs sequential, ~2.7μs under concurrent writers (mutex-serialized, so concurrency doesn't help throughput here). But this cost is not currently paid on any real write path: `MeshStore.Set`/`Consume`/`Seen` never call the WAL's append method, so persistence today only happens when `create_snapshot`/`restore_from_latest_snapshot` are called explicitly, not continuously — the benchmark measures what wiring the WAL onto the hot path would cost, a number that didn't exist before, precisely because that wiring doesn't exist yet. Redis's RDB/AOF persistence is battle-tested over 15+ years in production at massive scale; this project's persistence layer is both new and, per the above, not actually in the write path by default.
