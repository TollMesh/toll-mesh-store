...
- **Known bug, not yet fixed:** `SortedSet.Add`'s CRDT conflict resolution compares `(score, timestamp, node)` and treats any score *decrease* as a losing write, regardless of timestamp — so calling `zadd` with a lower score than the member currently has is silently ignored, even from the same node issuing a completely ordinary update (confirmed live). Unlike Redis's `ZADD`, which always sets the score you give it, this implementation currently cannot lower a member's score at all through the normal add path.
