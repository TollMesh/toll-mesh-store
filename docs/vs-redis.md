...
- Every successful `Set`/`Consume`/`Seen` now logs to the WAL (`PersistenceEngine.LogOperation`, ~4.4μs sequential, ~3.7μs under concurrent writers), and a fresh process automatically recovers full state on startup by loading the latest snapshot and replaying every WAL entry after it — verified live by hard-killing a running server (`SIGKILL`, no graceful shutdown) and confirming a new process on the same data directory recovered cache, replay-protection, and exact rate-limiter counts correctly. This was not true until recently: for most of this project's history "write-ahead log" was aspirational — the write path never called it, so only an explicit `create_snapshot` protected anything, and nothing replayed automatically on restart. Note this package actually contains three independent, mostly-redundant persistence implementations (`PersistenceEngine` in persistence.go, `WriteAheadLog`+`SnapshotManager`+`RecoveryManager` in wal.go/snapshot.go/recovery.go); only `PersistenceEngine` is wired to `MeshStore` — the others are exercised solely by their own unit tests. Recovery time and write-path overhead at real (non-benchmark) scale still haven't been measured; Redis's RDB/AOF persistence remains battle-tested over 15+ years in production at massive scale in a way this cannot yet claim.

---

## Architecture

**TollMeshCache**: peer-to-peer, CRDT-based. Each node holds full state; nodes gossip over the same HTTP API every SDK uses and merge incoming state via each primitive's real CRDT merge (`GCounter`/`GSet`). No coordinator, no leader election. This is a real, meaningful structural difference from Redis, and — unlike a lot of what's in this document's history — it's now genuinely working, not just unit-tested in isolation: verified live against three separately launched OS processes, including concurrent writes on different nodes and correct counter aggregation across the cluster. Only the original three primitives (rate limiting, replay protection, cache) replicate today; the eight feature groups added later (Job Queues, Sorted Sets, Streams, Pub/Sub, Transactions, Persistence, Pipelines, WASM Scripting, Search, Ranking, Metrics — this list grew over the project's history) all still live entirely within a single `MeshStore` instance's in-memory maps (plus, for Persistence, its own WAL/snapshot files), with no gossip replication wired to them. Cache replication is also a simplification rather than a full CRDT — see [Architecture: Gossip and state sync](architecture.md#gossip-and-state-sync) for what that means and why.
