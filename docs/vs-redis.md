...
**TollMeshCache**: peer-to-peer, CRDT-based. Each node holds full state; nodes gossip over the same HTTP API every SDK uses and merge incoming state via each primitive's real CRDT merge (`GCounter`/`GSet`, and cache's per-key LWW-register). No coordinator, no leader election. This is a real, meaningful structural difference from Redis, and — unlike a lot of what's in this document's history — it's now genuinely working, not just unit-tested in isolation: verified live against real, separately launched OS processes, including concurrent writes to the same key on different nodes converging correctly and correct counter aggregation across the cluster. Four primitives replicate today: the original three (rate limiting, replay protection, cache) plus, as of this session, Sorted Sets — the easiest of the eleven feature groups added after the original three to extend, since `SortedSet.Merge` already had a real CRDT conflict resolution built for local testing, just not wired to gossip yet. The other ten (Job Queues, Streams, Pub/Sub, Transactions, Persistence, Pipelines, WASM Scripting, Search, Ranking, Metrics) still live entirely within a single `MeshStore` instance's in-memory maps (plus, for Persistence, its own WAL/snapshot files), with no gossip replication wired to them, and most don't yet have a per-entry version concept to build that on top of. See [Architecture: Gossip and state sync](architecture.md#gossip-and-state-sync) for how cache's LWW-register merge works, a subtle durability bug it took a live crash test to surface and fix, and what Sorted Sets' replication reused versus what the remaining ten would need designed from scratch.
